...
exports.findByMSHS = async (req, res, next) => {
  try {
      const medicalrecordService = new MedicalrecordService(MongoDB.client);
      const medicalrecords = await medicalrecordService.findByMSHS(req.params.MSHS);
      // if (medicalrecords.length === 0) {
      //     return res.status(404).json({ message: "No medical records found for the provided MSHS" });
      // }
      res.json(medicalrecords);
  } catch (error) {
      console.log(error);
      next(new ApiError(500, "Error finding medical records by MSHS"));
  }
};
